...
export default function Skills({
  ...
  const fullStackCategories = useMemo<Category[]>(
    () => [
      {
        name: "frontend",
        label: "Frontend",
  ...
  const dataAICategories = useMemo<Category[]>(
    () => [
      {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && activeCategory) {
            const allCategories = [...fullStackCategories, ...dataAICategories];
  ...
  }, [skillsRef, activeCategory, animateSkillsSequentially, fullStackCategories, dataAICategories]);
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-300 text-2xl max-w-4xl mx-auto leading-relaxed">
            Comprehensive web development technologies
          </p>
        </div>
        ...
        <div className="text-center mb-16 mt-24">
          <h2 className="text-7xl md:text-9xl font-black text-transparent bg-gradient-to-r from-purple-400 via-purple-_forest, purple-600 to-teal-400 bg-clip-text mb-8 animate-gradient-x relative">
            DATA & AI
            <div className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 w-32 h-1 bg-gradient-to-r from-purple-400 to-purple-600 animate-shimmer"></div>
          </h2>
          <p className="text-gray-300 text-2xl max-w-4xl mx-auto leading-relaxed">
            Advanced AI and data science technologies
          </p>
        </div>

        {dataAICategories.map((category) => (
          <div key={category.name} className="mb-12">
            <button
              onClick={() => handleCategoryClick(category.name, category.skills)}
              onKeyDown={(e) => {
              ...
              aria-expanded={activeCategory === category.name}
              aria-controls={`skills-${category.name}`}
              className={`flex items-center space-x-4 w-full p-6 rounded-2xl transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-purple-400 ${
                activeCategory === category.name
                  ? `bg-gradient-to-r ${category.color} text-white shadow-lg shadow-${category.color.split(" ")[1]}/25`
                  : "bg-gray-800/50 hover:bg-gray-700/50"
              }`}
